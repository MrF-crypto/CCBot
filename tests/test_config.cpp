...
int main() {
    ...
    // ── ④b 扫描器段 ─────────────────────────────────────────────────────────
    {
        std::printf("\n── 全市场扫描器配置 ──\n");
        write_file(path,
            "{\"api_key\":\"k\",\"api_secret\":\"s\",\"max_open_positions\":6,"
            "\"bots\":[{\"symbol\":\"BTCUSDT\"}],"
            "\"scanner\":{\"enabled\":true,\"interval_secs\":90,"
            "\"min_quote_vol_24h\":80000000,\"coarse_top_n\":40,"
            "\"max_pct_b\":-0.1,\"max_rsi\":28,\"final_top_n\":6,"
            "\"budget_per_position\":300,"
            "\"blacklist\":[\"AAAUSDT\",\"BBBUSDT\"],"
            "\"template\":{\"entry_mode\":\"immediate\",\"max_entries\":3,"
            "\"tp_floor_only\":true,\"tp_fixed_profit\":1.5,\"fixed_trail_tp\":0.3,"
            "\"leverage\":5}}}");
        HeadlessConfig hc; std::string err;
        check(load_headless_config(path, hc, err), "扫描器配置能解析: " + err);
        const auto& S = hc.scanner;
        eq(S.enabled, true,                "scanner.enabled");
        eq(S.interval_secs, 90,            "interval_secs");
        eqd(S.cfg.min_quote_vol_24h, 8e7,  "min_quote_vol_24h");
        eq(S.cfg.coarse_top_n, 40,         "coarse_top_n");
        eqd(S.cfg.max_pct_b, -0.1,         "max_pct_b");
        eqd(S.cfg.max_rsi, 28.0,           "max_rsi");
        eq(S.cfg.final_top_n, 6,           "final_top_n");
        eq((int)S.cfg.blacklist.size(), 2, "黑名单 2 个");
        check(S.cfg.blacklist.count("AAAUSDT") == 1, "  黑名单内容正确");
        eq(hc.max_open_positions, 6,       "max_open_positions（账户级）");

        // 模板走的是与普通 bot 完全相同的解析路径
        eq(S.templ.entry_mode, CcgConfig::EntryMode::Immediate, "模板 entry_mode=immediate");
        eq((int)S.templ.max_entries, 3,    "模板 max_entries");
        eq((int)S.templ.leverage, 5,       "模板 leverage");
        eq(S.templ.tp_floor_only, true,    "模板 tp_floor_only（快进快出）");
        eqd(S.templ.tp_fixed_profit, 1.5,  "模板 tp_fixed_profit");
        eqd(S.templ.fixed_trail_tp, 0.3,   "模板 fixed_trail_tp");
        eqd(S.templ.budget_usdt, 300.0,    "budget_per_position 覆盖模板预算");
        // 扫描出的仓位止盈后必须让位，否则一直占着并发额度
        check(S.templ.auto_restart == false, "模板 auto_restart 被强制为 false");
        bool warned = false;
        for (const auto& w : hc.warnings)
            if (w.find("auto_restart") != std::string::npos) warned = true;
        check(warned, "  并且给出了明确告警");
    }

    // ── ④c 开扫描器却没设并发上限 = 致命配置错误 ────────────────────────────
    // 全市场扫描下大跌那天可能几十个品种同时触发，没有这道闸就是灾难
    {
        write_file(path,
            "{\"api_key\":\"k\",\"api_secret\":\"s\","
            "\"bots\":[{\"symbol\":\"BTCUSDT\"}],"
        ...
        check(!load_headless_config(path, hc, err),
              "开 scanner 但没设 max_open_positions → 拒绝启动");
        check(err.find("max_open_positions") != std::string::npos, "  错误信息指明了缺什么");
    }

    // ── ④d 扫描器模式下 bots 可以为空 ───────────────────────────────────────
    {
        write_file(path,
            "{\"api_key\":\"k\",\"api_secret\":\"s\",\"max_open_positions\":8,"
            "\"bots\":[],\"scanner\":{\"enabled\":true}}");
        HeadlessConfig hc; std::string err;
        check(load_headless_config(path, hc, err),
              "扫描器模式下 bots 为空是合法的（品种全由扫描器产生）: " + err);
    }
