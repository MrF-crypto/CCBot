...
struct CcgConfig {
    ...
    // 下面那段"放大倍数会让利润塌掉"的记录是【旧配置】（宏观拦截开/7层/保底3.5%）
    // 下测的，在当前配置（闸门关/8层/保底2.0%/下轨首单）下不再成立——
    // 当前配置里 mult 2.0~5.0 普遍优于 1.0，只是仍然不如固定间隔。
    // 实测（BTC六年，8层/保底2.0%/宏观拦截）：满层时间占比与回撤确实随倍数
    ...
    //   24h涨幅 htf_24h_chg_max  —— 涨太急不追
    //   7日涨幅 htf_week_chg_max —— 中期涨太多不追
    //
    ...
    // ── 补仓侧闸门（v3.3 实验）────────────────────────────────────────────────
    // 宏观许可层目前只管首仓，而递增7层曲线下首仓仅占预算 1/28≈3.6%，第5~7层占
