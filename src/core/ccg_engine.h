...
struct CcgConfig {
    ...
    // ── 保底利润随层数递减 ⚠ 已被实测证伪，保留只为记录结论 ───────────────────
    // 有效保底 = min_profit_floor × (1 − decay × (已用层数−1)/(max_entries−1))
    //
    // 原始动机是一个数学事实：满层时止盈难度是单层的十倍量级。
    // 设入场 100、跌到 70 满层、均价 85：
    //   单层  均价100 → 止盈线 102  → 需涨  2%
    //   满层  均价 85 → 止盈线 86.7 → 从当前价(70)需涨 24%
    // 由此推断"满层时该降低要求换取脱身"。
    //
    // 实测单调恶化，8 品种无一例外（2022-01~2026-08，各自最优间隔）：
    //   decay 0.00  合计 137175  比率 3.60  满层 2.40%   ← 现行
    //   decay 0.40  合计 132696  比率 3.46  满层 2.38%   −3.3%
    //   decay 0.60  合计 129143  比率 3.33  满层 2.38%   −5.9%
    //   decay 0.75  合计 126633  比率 3.27  满层 2.38%   −7.7%
    //
    // 错在【前提】而不是推导：最优配置下满层率本来就只有 2.4%，
    // "如何从满层脱身"不是真问题——那个问题已经被"避免满层"解决掉了。
    // 于是 decay 的实际影响全部落在中间层（3~5层），在那里降低止盈要求
    // 导致过早平仓，纯损失。满层率一列几乎不动（2.40→2.38）正是这个的证据。
    //
    // 更一般的教训见 tp_floor_only：这套策略的钱靠"等到位"赚，不靠周转赚。
    // 任何降低出场标准的改动都要先过这一关。
    double floor_decay = 0.0;

    // 补仓间隔倍数（1.0=原始设计 W/3，夹逼[0.3,1.5]）。放大它等于把梯子摊到
    // 更宽的价格带上——见 dynparams::interval_pct 的说明。
    // ⚠ 这整条路径已被实测判负：任何倍数都打不过固定间隔，详见 dyn_fixed_interval。
    // 下面那段"放大倍数会让利润塌掉"的记录是【旧配置】（三层拦截开/7层/保底3.5%）
    // 下测的，在当前配置（闸门关/8层/保底2.0%/下轨首单）下不再成立——
    // 当前配置里 mult 2.0~5.0 普遍优于 1.0，只是仍然不如固定间隔。
    ...
    // ── 补仓是否必须在带外（默认 true = 现行行为）────────────────────────────
    // 动态W 的"结构锚定"有两半，方向未必对称：
    //   止盈锚上轨 —— 已验证是核心价值（去掉损失 48.5%）。等价格真的回到统计
    //                 高位再走，筛掉的是"只反弹一点就跑"的零碎周期。
    //   补仓锚下轨 —— 从未单独验证。它要求"跌够间隔 AND 价格在下轨外"两个条件
    //                 同时满足才补仓。
    // 怀疑点：间隔已经取到 6%（很宽），下轨这个附加条件可能在过度限制补仓——
    // 持续下跌时带子跟着价格下移，价格未必总在带外，于是该补的没补上，
    // 均价压不下来。止盈时"等极端位"是有价值的等待，补仓时"等极端位"
    // 可能是有害的错过。
    bool dca_require_band = true;
    ...
    // 固定补仓间隔%（0=用 W/3 推导）。当初加它是为了回答一个悬置的问题：
    // 动态W的"自适应"本身有没有创造价值？
    //
    // ⚠ 答案已经实测出来了：【固定间隔全面胜出，自适应是负贡献】。
    //
    // 六年全量、8品种、闸门关/8层/递增/保底2.0%/下轨首单/趋势过滤开：
    //   · 跨品种扫固定间隔 2~8%：7品种 × 6档 = 42 组【全部正收益】，
    //     净利中位在 6% 达峰（33594），4~6% 都是稳健区
    //   · 动态W扫倍数 1~5（5品种）：任何倍数都打不平该品种的最优固定间隔，
    //     现行默认 mult=1.0 差 16%~78%、满层中位 84.5%，0/5 品种能打平；
    //     最好的倍数也只有 1/5 打平，且各品种最优倍数不一致（2.0~5.0），
    //     参数面非单调（ADA 在 mult=3.0 崩到 3071，两侧都是 3 万+）
    //
    // 为什么自适应反而更差：布林带宽 W 描述的是【1h 级别的近期波动】，
    // 而补仓决策关心的是【日/周级别的回撤深度】——时间尺度不匹配。
    // 更糟的是方向：暴跌时 W 扩大（间隔变宽，正确），但暴跌企稳后 W 收缩
    // （间隔变窄），于是在底部区域密集补仓，恰好是最该省弹药的时候。
    //
    // 最优固定值跨品种漂移（BNB/XRP 3%、DOGE/LINK/SOL 4%、ETH 5%、ADA 6%），
    // 且与带宽中位的比值也不稳定（0.74~1.48），所以【没有可映射的通用公式】。
    //
    // ⚠ walk-forward 给出了最终答案，并推翻了上面那些"最优值"：
    // 8品种 × 8 滚动窗口（预热4月/训练12月/测试6月，前进6月，共 64 个纯样本外窗口）
    //
    // 各固定间隔在【全部测试段】的成绩：
    //    3%   总净利 −35616   盈利 38/64   满层中位 22.4%   ← 样本外净亏
    //    4%   总净利  53826   盈利 49/64   满层中位  1.2%
    //    5%   总净利  67289   盈利 52/64   满层中位  0.7%
    //    6%   总净利  70620   盈利 56/64   满层中位  0.0%   ← 最优
    //    7%   总净利  42794   盈利 55/64   满层中位  0.0%
    // 「BTC 上 4% 最优」是单品种单区间的样本内产物，样本外不成立；6% 才是。
    //
    // 更重要的是【调参本身有害】：
    //    每窗口选训练段最优 → 测试段  总净利 65827  盈利 48/64
    //    从头到尾固定 5%（不调参）    总净利 67289  盈利 52/64
    //    事后最优（不可实现的上界）   总净利 178218 盈利 60/64
    // 选最优 / 固定值 = 0.98——逐窗口挑最优还不如一个固定值。
    // 原因看分布就清楚：训练段有 55% 的窗口选中最窄的 3%（样本内周期多、
    // 数字好看），而 3% 恰恰是样本外唯一净亏的一档。
    //
    // 样本外衰减：训练段月均 511U → 测试段月均 171U，留存 34%（中位 48%）。
    // 本文件里所有回测数字都是样本内的，实盘预期应按此折价。
    double dyn_fixed_interval = 0.0;

    // ── 梯度间隔（0=关，等距）⚠ 已被实测证伪，保留只为记录结论 ────────────────
    // 第 k 层的补仓间隔 = 基础间隔 × (1 + growth × (k-1))，越深的层要求跌得越多。
    //
    // 原始动机是两组数据看起来的矛盾：入场后 14 天中位再跌 7%、P90 再跌 20~25%
    // （梯子得覆盖到 P90），而等距 4% 下平均只用到 1.65 层（看起来前段太疏、
    // 浅回调吃不到）。梯度间隔试图前段密、后段疏，同时满足两边。
    //
    // 实测彻底否定（BTC 六年，预算18000/8层/递增/保底2.0%，对照固定4%）：
    //   固定4%            净利 14470  回撤  3491  比率 4.14  满层  2.2%
    //   基础2%  growth0.5 净利  9232  回撤  8332  比率 1.11  满层 13.8%
    //   基础2.5%growth0.4 净利  5845  回撤  7213  比率 0.81  满层 16.0%
    //   基础3%  growth0.2 净利  5748  回撤 12996  比率 0.44  满层 37.3%
    //   基础1.5%growth0.5 净利  3846  回撤 13327  比率 0.29  满层 48.8%
    //   基础2%  growth0.3 净利  3073  回撤 13338  比率 0.23  满层 49.1%
    // 规律干净：基础间隔越小 → 满层占比越高 → 表现越差。最好的梯度版本也只有
    // 固定4%的四分之一。
    //
    // 错在把因果搞反了：「平均只用 1.65 层」不是浪费，正是它成功的原因。
    // 固定4%的前段就是4%，浅回调【根本触发不了补仓】，所以大部分时间只持有
    // 1 层、弹药没动过。前段调密之后，浅回调把前几层打出去，一进半仓状态
    // 再遇深跌就满层——而满层占比是驱动回撤和资金费的枢纽变量。
    //
    // 由此得到一个更重要的认识：4% 这个最优配置【不是网格】，是
    // 「下轨信号开首仓 → 快速止盈 → 再开仓」的低频择时循环（六年 417 个周期）。
    // 补仓层数是保险，不是利润来源；每次补仓都意味着首仓的判断错了。
    // "网格赚的是小幅震荡的钱"这句话只对窄间隔配置（动态W，约0.9%）成立。
    double dyn_interval_growth = 0.0;
    ...
    // 默认 0=关。阈值取值不在本仓库验证（回测已移出为独立项目）
    // ⚠ v4.0.9 起这一条改用【24 小时滚动涨幅】（币安 @ticker 的 P 字段，
    //   就是界面上那个"24h涨幅"），不再是"今日涨幅"。
    //   原因：今日涨幅 = 现价相对上一根已收盘日线，每天 UTC 0 点归零——
    //   而 UTC 0 点是北京时间早 8 点，是真实交易时段。某币 20:00–23:00 拉了 50%，
    //   23:30 时闸门拦得住，00:30 就放行了，币还在一天前的 1.5 倍位置上。
    //   归零时刻与拉盘时刻的相对位置基本随机，等于闸门有效性靠运气
    double      htf_24h_chg_max    = 0;       // 0=关；24h涨幅超过此% 拦新首仓（做多）
    double      htf_week_chg_max   = 0;       // 0=关；近7根日线涨幅超过此%
    bool        use_sr_support     = true;    // 结构①：脚下踩住够格支撑区
    bool        use_sr_headroom    = true;    // 结构②：头顶净空足够止盈
    int         sr_min_confluence  = 2;       // 够格区域的最低共振数
    // ⚠ 已移除的实验项 sr_res_min_conf（阻力侧单独计票门槛）——结论存档：
    //   动机是同门槛下两侧不对称：支撑票数不够=拦（保守），阻力票数不够=这堵墙
    //   被忽略、净空显示∞=放行（激进）。但降到 1 票实测是灾难：净空3.0 下周期
    //   从 47 掉到 7、收益 1519→-17；4段 walk-forward 最好的一档(净空2.0)也只有
    //   0.252 vs 基线 0.548。够格阻力一多，净空比处处偏小，3.0 门槛几乎筛掉全部
    //   机会。不对称是真的，但"两侧都保守"并不等于更好。
    //   它此前只有回测能设，且生产代码从未把它读进 DigestOpts——回测移出后
    //   成了双重死配置，v4.0.13 删除
    ...
    // ⚠ 已移除的实验项 sr_headroom_true_tp（净空分母取"上轨距离与保底线的远者"）
    //   ——结论存档：动机看似成立（止盈是双条件，只用上轨距离会低估要走的路），
    //   但实测更差：19个月连续 1519→1184、收益/回撤 2.26→1.76、周期 47→40；
    //   把净空门槛一起重扫(1.0~4.0)后最好一档也只有 0.452 vs 基线 0.548。
    //   论证错在哪：保底线是 avg×(1+floor)，而 avg 会被补仓一路压低，真正的止盈
    //   【价格】常常低于入场价×(1+floor)。用入场价×floor 当分母等于假设了一个
    //   没有补仓的世界，对 DCA 策略是系统性高估。"到上轨距离"随行情实时更新，
    //   反而是更好的估计量。
    //   唯一可取处：它是全部正收益组合里回撤最低的一条（净空2.5 时回撤 1434
    //   vs 基线 1973）。默认 false 且回测移出后已无任何入口，v4.0.13 删除
    // ── 快进快出方案（v3.3 实验）─────────────────────────────────────────────
    // 现行动态W止盈是【触上轨 且 盈利≥保底】双条件，本质是"等一个完整的带内摆动"。
    // 另一种思路是"够本就跑"：不等上轨，盈利达标即激活追踪，靠高周转取胜。
    // tp_floor_only=true 时止盈激活【只看保底利润】，不要求触上轨（=替换上轨条件）
    //
    // ⚠ 实测：这条思路是错的，"触上轨"不是障碍而是【核心价值来源】。
    // 8 品种合计净利（2022-01~2026-08，各自最优间隔）：
    //   触上轨+保底2%（现行）  137175   比率 3.60   周期 326
    //   仅保底3%                86718   比率 2.06   周期 311   −36.8%
    //   仅保底2%                70691   比率 1.70   周期 478   −48.5%
    //   仅保底1%                34455   比率 1.04   周期 892   −74.9%
    //
    // 去掉上轨条件后周期数涨到 2.7 倍，净利却只剩四分之一——它筛掉的正是那些
    // "只反弹了一点点就跑"的零碎周期。触上轨的含义是【价格真的回到了统计高位】，
    // 也就是这一轮均值回归确实完成了；保底利润只是防止在下跌趋势里亏着平仓的兜底。
    //
    // 由此得到一条贯穿性的结论：**这套策略的钱靠"等到位"赚，不靠周转赚。**
    // 同一方向的旁证：保底 3% 优于 1%（更少更肥的周期）；立即开仓虽然把周期数
    // 提高 37% 却让总收益掉 14.3%（见 EntryMode）；floor_decay 降低深层出场
    // 标准同样是负贡献。任何"降低出场标准换取周转"的改动都要先过这一关。
    ...
    // 固定【补仓】追踪建仓%（0=用动态W推导的 clamp(0.10W, 0.15, 0.4)）。
    // 与 fixed_trail_tp 对称。存在的理由：动态推导的上限只有 0.4%，而间隔一旦
    // 换成宽的固定值（比如 4%），0.4% 的反弹确认相对间隔就太小了——跌够 4% 之后
    // 随便一个 0.4% 的反弹就补进去，等于没等企稳。两者应当成比例，不能一个宽一个窄
    double      fixed_trail_entry = 0.0;
...
struct CcgBot {
    ...
    bool   htf_chg_ok    = false;   // 7日涨幅是否算得出（历史不足 8 根时为 false）
    double htf_week_chg  = 0;       // 近7根日线涨幅%
    std::chrono::steady_clock::time_point htf_time{};
    // 24h 滚动涨幅走另一条路：来自 @ticker 推送流，不是日线K线。
    // 新鲜度与 htf_time 分开记——两条流的更新频率差两个数量级
    bool   h24_ok        = false;
    double h24_chg       = 0;
    std::chrono::steady_clock::time_point h24_time{};
...
public:
    ...
    // chg_ok/week_chg 有默认值：调用方拿不到日线数据时可以只传 %B
