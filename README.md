...
v4.1.0 起本程序同时提供两套风险形状**镜像**的策略。界面上是两个标签页，配置文件里是两个数组，**同一个品种只能二选一**（两套会在同一个交易所仓位上互相平掉对方的单，程序直接拒绝）。

| | **网格 DCA** | **趋势 SAR** |
|---|---|---|
| 入场 | 立即 / BOLL+RSI 信号 | 唐奇安通道突破 |
| 加仓 | 分层摊薄（最多 50 层） | 单仓位，不加仓 |
| 出场 | 追踪止盈 | **ATR 追踪止损，无固定止盈** |
| 亏损 | 不止损，浮亏扛着 | 每笔都止损 |
| 胜率 | 高（80%+） | **低（30~40%）** |
| 盈利来源 | 大量小赢 | **少数几笔跑很远** |
| 致命场景 | 单边暴跌打光梯子 | **震荡市连续磨损** |
| 核心不变量 | 名义仓位 ≤ 权益 ⇒ 不可强平 | 不适用 |

两套引擎互不依赖，只共用行情客户端与线程池。
...
| | |
|---|---|
| **入场信号** | 唐奇安通道突破：上破 N 根最高 → 做多，下破 N 根最低 → 做空。通道**只由已收盘K线构成**，当前这根去撞它——把当前根算进通道等于「价格≥上沿」恒成立、信号恒真 |
| **出场** | Chandelier ATR 追踪止损：`持仓期极值 ∓ k×ATR`，棘轮，只朝有利方向移动。ATR 用 Wilder 平滑，与 TradingView / 币安同口径 |
| **无固定止盈** | 这是设计而非遗漏。趋势跟随胜率天然只有 30~40%，收益全来自少数跑得很远的单子；固定止盈砍断它们而亏损笔大小不变，等于单方面砍掉盈利分布的右尾 |
| **止损后反转** | 亏损出场可反向入场；**盈利出场永不反手**——力竭不等于反转 |
| **反手信号闸** | 默认开启。要求反向信号真的成立，而不是「我被打了所以反着来」。无条件反手在震荡市是绞肉机：ATR 常态 2% 时单次绞杀约 6% 名义，3 倍杠杆即保证金的 18% |
| **连续反手上限** | 默认 2 次，超过强制冷却若干根 K 线。真趋势不需要连续反手——连续反手本身就是「现在是震荡市」的信号 |
| **按 ATR 等风险下单** | 名义 = `单次愿亏 / (k×ATR%)`，每笔止损亏的钱固定。同一份名义在 ATR 1.6% 和 5.3% 的品种上，单次亏损差 3.3 倍——固定名义等于把风险全压在高波动那几个上，「铺开品种分散风险」就此失效。海龟的「单位」概念 |
| **顺势加仓（金字塔）** | 每朝有利方向再走 N 个 ATR 加一档。**错了只亏第一档，对了越骑越重**——这是「低成本试出单边大行情」的标准答案。与 DCA 的补仓方向相反 |
| **跨重启** | 持仓、开仓价、止损线、反手计数全部落盘；启动与运行中各与交易所对账。**本地空仓而交易所有仓会直接停 bot**——不停的话下一个信号会再开一笔，净敞口翻倍 |

界面里「距离」列是持仓时最该盯的数：现价离止损线还有多远，**止损线越过成本价后转绿**（这笔已锁定盈利）。「ATR」列同时是数据指示灯：显示 `—` 标红就是 K 线没拉到，此时没有止损线、引擎不会开仓；有数值时悬停给出当前 k 对应的实际止损距离，用来定 k。

### 开仓闸门（DCA 专用，层层递进，任一不满足就不开新首仓）
