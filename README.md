...
- **SR雷达**（v2.6+，影子模式）：自动检测支撑/阻力区域——摆动点分形聚类（ATR自适应容差+触碰次数×时间衰减×成交量评分+攻防转换加成）+ FVG公允价值缺口，约15分钟滚动刷新、区域自然生灭。价格触区打日志+webhook告警，右键品种可查看区域列表。当前为影子模式（只观察不下单），用于验证检测质量后再接执行。
