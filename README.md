...
- **SR雷达**（v2.6+，影子模式）：自动检测支撑/阻力区域——四类来源共同入池：摆动点分形聚类（ATR自适应容差+触碰×时间衰减×成交量评分+攻防转换加成）、FVG公允价值缺口、POC筹码峰（成交量分布主/次峰）、斐波那契回撤位（0.5/0.618/0.786）。**跨来源共振合并**（v2.7+）：不同类型点位叠在同一价位自动合并，共振数加权（"≥2种点位共振才值得动手"的程序化），告警与区域表直接显示"摆动+FVG ×2共振"。约15分钟滚动刷新、区域自然生灭；影子模式只观察不下单。
...
- **持久化（便携模式，v2.8+）**：策略配置、交易明细、API 凭证、日志全部保存在**程序目录的 `data/` 子目录**——整个程序文件夹拷走就是完整备份（凭证仍为 Windows DPAPI / macOS 钥匙串加密，换机需重新输入）。老版本 AppData 数据首次启动自动迁移；程序目录不可写时自动回退 AppData。运行状态原子写盘防崩溃损坏。
...
**v4.0.13 起回测由独立项目承担，不再包含在本仓库内。**

本仓库只保留一条真相：实盘逻辑。回测需要重新实现指标推进、K线聚合、撮合、资金费，
与实盘之间任何一处偏差都会让结论悄悄失真——把它拆出去之后，这里不再存在需要
同步维护的第二份策略逻辑。

现行默认参数的实证依据（净空比 3.0 / 日线%B 0.60 / 保底利润 2.0% / 层数 10 等）
仍记录在 [docs/BACKTEST.md](docs/BACKTEST.md) 与各处代码注释里。
