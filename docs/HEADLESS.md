...
> ⚠ **SAR 仓位目前不落盘。** 进程重启后本地状态会丢，而交易所上的仓位还在。重启前请手动平掉 SAR 仓位，或只在测试网跑。这是当前最大的缺口。
...
**选 `atr_mult` 前先看实测 ATR**：GUI 版每品种每 10 轮会打一条 `ATR观测` 日志，直接给出 k=2.5/3.0/3.5 对应的止损距离百分比。跨品种唯一可比的口径是 `atr/price`，所以 BTC 和新上山寨能共用同一组参数。
